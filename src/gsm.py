...
class gsmModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, *vargs, **kwargs):
        super(gsmModule, self).__init__()

        # self.conv1_1 = nn.Conv1d(in_channels=64, out_channels=2, kernel_size=1)
        self.conv3D = nn.Conv3d(in_channels, out_channels, kernel_size, stride=1,
                                padding=(0, 2, 2), groups=2, *vargs, **kwargs)
        nn.init.constant_(self.conv3D.weight, 0)
        nn.init.constant_(self.conv3D.bias, 0)
        self.tanh = nn.Tanh()
        self.fPlane = in_channels
        self.num_segments = 3
    ...
    def forward(self, x):
        ...
        x = x.view(batchSize,x.size(1), *shape).permute(0, 2, 1, 3, 4).contiguous()
        x_bn = self.bn(x)
